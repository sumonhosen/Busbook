...
class SslCommerzPaymentController extends Controller
{
    ...
    {
        $booking = TripBookingListModel::where('token',$request->booking_token)->first();

        $trip_break_down = TripBreakdownModel::with('bus')->where('token',$request->break_token)->first();

        $booking_seats = BookingSeatModel::where('bus_token',$trip_break_down->bus->token)->where('booking_token',$request->booking_token)->get();
        ...
        $post_data = array();
        $post_data['total_amount'] = $final_fare;
